...
export function Prompt(props: PromptProps) {
  ...
  // Hermes proxy: extract target agent from first user message's AgentPart
  const proxyTarget = createMemo(() => {
    if (!props.sessionID) return undefined
    const messages = sync.data.message[props.sessionID]
    if (!messages) return undefined
    const firstUser = messages.find((m): m is UserMessage => m.role === "user")
    if (!firstUser) return undefined
    const parts = sync.data.part[firstUser.id]
    if (!parts) return undefined
    const agentPart = parts.find((p: { type: string; name?: string }) => p.type === "agent")
    if (!agentPart || !("name" in agentPart)) return undefined
    return (agentPart as { type: string; name: string }).name
  })
