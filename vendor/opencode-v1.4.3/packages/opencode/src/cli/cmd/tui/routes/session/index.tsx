...
export function Session() {
  ...
  const sessionProxyTarget = createMemo(() => {
    ...
    const agentPart = parts.find((p: { type: string }) => p.type === "agent")
  ...
  return (
    <context.Provider
      value={{
        ...
        sessionID: route.sessionID,
        proxyTarget: sessionProxyTarget,
        conceal,
        showThinking,
        showTimestamps,
        showDetails,
...
function UserMessage(props: {
  ...
  // Hermes proxy: derive display agent from first user message AgentPart
  const proxyAgent = createMemo(() => {
    const agentPart = props.parts.find((x: { type: string }) => x.type === "agent")
    if (agentPart && "name" in agentPart) return (agentPart as { name: string }).name
    return undefined
  })
  const color = createMemo(() => local.agent.color(proxyAgent() ?? ctx.proxyTarget() ?? props.message.agent))
                ...
                style={{
                  fg:
                    props.message.error?.name === "MessageAbortedError"
                      ? theme.textMuted
                      : local.agent.color(ctx.proxyTarget() ?? props.message.agent),
                }}
              >
                ▣{" "}
              </span>{" "}
              <span style={{ fg: theme.text }}>{Locale.titlecase(ctx.proxyTarget() ?? props.message.mode)}</span>
