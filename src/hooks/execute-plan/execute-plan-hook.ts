...
All ${progress.total} tasks are done. Create a new plan with Mnemosyne or Prometheus.`
            ...
            const newState = createBoulderState(matchedPlan, sessionId, "heracles", worktreePath)
...
No plan files found at .sisyphus/plans/
...
All ${plans.length} plan(s) are complete. Create a new plan with Mnemosyne or Prometheus.`
          ...
          const newState = createBoulderState(planPath, sessionId, "heracles", worktreePath)
          writeBoulderState(ctx.directory, newState)
