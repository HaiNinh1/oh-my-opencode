...
describe("category-skill-reminder hook", () => {
  ...
  describe("target agent detection", () => {
    test("should inject reminder for sisyphus agent after 3 search tool calls", async () => {
      //#given: sisyphus agent session with multiple search tool calls
      ...
      //#when: 3 grep tool calls are made
      ...
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "3" }, output)

      //#then: reminder should be injected
      expect(output.output).toContain("[Explore/Librarian Reminder]")
    ...
    test("should inject reminder for atlas agent", async () => {
      ...
      //#when: 3 glob tool calls are made
      await hook["tool.execute.after"]({ tool: "glob", sessionID, callID: "1" }, output)
      await hook["tool.execute.after"]({ tool: "glob", sessionID, callID: "2" }, output)
      await hook["tool.execute.after"]({ tool: "glob", sessionID, callID: "3" }, output)
    ...
    test("should inject reminder for sisyphus-junior agent", async () => {
      ...
      //#when: 3 webfetch tool calls are made
      await hook["tool.execute.after"]({ tool: "webfetch", sessionID, callID: "1" }, output)
      await hook["tool.execute.after"]({ tool: "webfetch", sessionID, callID: "2" }, output)
      await hook["tool.execute.after"]({ tool: "webfetch", sessionID, callID: "3" }, output)
    ...
    test("should NOT inject reminder for non-target agents", async () => {
      ...
      //#when: 3 search tool calls are made
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "1" }, output)
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "2" }, output)
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "3" }, output)

      //#then: reminder should NOT be injected
      expect(output.output).not.toContain("[Explore/Librarian Reminder]")
    ...
    test("should detect agent from input.agent when session state is empty", async () => {
      ...
      //#when: 3 grep tool calls with agent in input
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "1", agent: "Sisyphus" }, output)
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "2", agent: "Sisyphus" }, output)
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "3", agent: "Sisyphus" }, output)
  ...
  describe("non-search tools should not trigger reminder", () => {
    test("should NOT inject reminder for edit/write/bash/read tools", async () => {
      //#given: sisyphus agent using implementation tools
      const hook = createHook()
      const sessionID = "impl-session"
      updateSessionAgent(sessionID, "Sisyphus")

      const output = { title: "", output: "result", metadata: {} }

      //#when: many edit/write/bash/read calls are made
      ...
      await hook["tool.execute.after"]({ tool: "read", sessionID, callID: "4" }, output)
      await hook["tool.execute.after"]({ tool: "edit", sessionID, callID: "5" }, output)

      //#then: reminder should NOT be injected (these are implementation tools, not search)
      expect(output.output).not.toContain("[Explore/Librarian Reminder]")
  ...
  describe("delegation tool tracking", () => {
    test("should NOT inject reminder if task is used", async () => {
      ...
      //#when: task is used, then more search tool calls
      await hook["tool.execute.after"]({ tool: "task", sessionID, callID: "1" }, output)
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "2" }, output)
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "3" }, output)
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "4" }, output)
    ...
    test("should NOT inject reminder if task tool is used", async () => {
      ...
      await hook["tool.execute.after"]({ tool: "glob", sessionID, callID: "2" }, output)
      await hook["tool.execute.after"]({ tool: "glob", sessionID, callID: "3" }, output)
      await hook["tool.execute.after"]({ tool: "glob", sessionID, callID: "4" }, output)
  ...
  describe("tool call counting", () => {
    test("should NOT inject reminder before 3 search tool calls", async () => {
      //#given: sisyphus agent with only 2 search tool calls
      const hook = createHook()
      ...
      //#when: only 2 grep calls are made
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "1" }, output)
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "2" }, output)
    ...
    test("should only inject reminder once per session", async () => {
      ...
      //#when: 6 search tool calls are made (should trigger at 3, not again at 6)
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "1" }, output1)
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "2" }, output1)
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "3" }, output1)
      await hook["tool.execute.after"]({ tool: "glob", sessionID, callID: "4" }, output2)
      await hook["tool.execute.after"]({ tool: "glob", sessionID, callID: "5" }, output2)
      await hook["tool.execute.after"]({ tool: "glob", sessionID, callID: "6" }, output2)

      //#then: reminder should be in output1 but not output2
      expect(output1.output).toContain("[Explore/Librarian Reminder]")
      expect(output2.output).not.toContain("[Explore/Librarian Reminder]")

      clearSessionAgent(sessionID)
    })

    test("should only count search tools", async () => {
      ...
      //#when: non-search tools are called (should not count)
  ...
  describe("event handling", () => {
    test("should reset state on session.deleted event", async () => {
      ...
      expect(output2.output).toContain("[Explore/Librarian Reminder]")
  ...
  describe("case insensitivity", () => {
    test("should handle tool names case-insensitively", async () => {
      ...
      //#when: search tool calls with different cases
      await hook["tool.execute.after"]({ tool: "GREP", sessionID, callID: "1" }, output)
      await hook["tool.execute.after"]({ tool: "Grep", sessionID, callID: "2" }, output)
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "3" }, output)
  ...
  describe("dynamic skills reminder message", () => {
    test("shows built-in skills in reminder", async () => {
      //#given: only built-in skills available
      ...
      //#when: 3 search tool calls
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "1" }, output)
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "2" }, output)
      await hook["tool.execute.after"]({ tool: "grep", sessionID, callID: "3" }, output)

      //#then: reminder should contain skill names
      expect(output.output).toContain("Built-in:")
      expect(output.output).toContain("frontend-ui-ux")
    })

    test("includes user skills in reminder message", async () => {
      //#given: both built-in and user skills
      const availableSkills: AvailableSkill[] = [
        { name: "frontend-ui-ux", description: "Frontend UI/UX work", location: "plugin" },
      ...
      //#then: reminder should contain user skill names
    ...
    test("still injects a generic reminder when no skills are provided", async () => {
      ...
      //#then: generic reminder should still be injected
      expect(output.output).toContain("[Explore/Librarian Reminder]")
    })
  })
})
