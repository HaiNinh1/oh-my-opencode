...
3. **Tests** - run tests adjacent to changed files. If no focused test exists, run the smallest relevant test command available or explicitly report why no test command could be run. Actually pass, not "should pass."
4. **Build** - if applicable, exit 0.
5. **No routine git proof** - do not run \`git diff\` or \`git status\` just to prove completion unless the task is explicitly about git, history, branch state, PR review, or regression investigation.
6. **Manual QA Gate** - when there is runnable or user-visible behavior, run the lightest reliable check through its matching surface yourself: \`interactive_bash\` for TUI/CLI, \`curl\` for HTTP, driver script for library/SDK. Browser automation is opt-in verification, not routine post-work verification: use it only when the user explicitly asks for browser/UI QA, the change affects browser-rendered UI, or non-browser checks cannot prove the behavior. Do not use \`agent-browser\` on Windows unless the user explicitly requested it; prefer project tests, build/typecheck, component/E2E commands, \`curl\`, or a driver script and report when browser QA was intentionally skipped. \`lsp_diagnostics\` catches type errors, not logic bugs; tests cover only what their authors anticipated. "Should work" is not verification.
