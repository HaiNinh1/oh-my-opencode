...
import { EXECUTE_PLAN_TEMPLATE } from "../../features/builtin-commands/templates/execute-plan"
import {
  readBoulderState,
  writeBoulderState,
  appendSessionId,
...
export const createResolveHeraclesContextTool = (
  directory: string,
  sessionId: string
): ToolDefinition =>
  tool({
    description: "Resolves dynamic context for starting work with Heracles. Handles Prometheus plan discovery, selection, and boulder state initialization. Use this when routing to Heracles with a plan request.",
    args: {
      planName: tool.schema.string().describe("Explicit plan name or keywords to search for (e.g., 'dark mode', 'payment gateway'). NOT a generic request sentence like 'Tell heracles to execute...'. Extract only the actual plan name/keywords."),
    },
    execute: async (args) => {
      try {
...
All ${progress.total} tasks are done. Create a new plan with: /plan "your task"`
              ...
              const newState = createBoulderState(matchedPlan, sessionId, "heracles")
...
All ${plans.length} plan(s) are complete. Create a new plan with: /plan "your task"`
            ...
            const newState = createBoulderState(planPath, sessionId, "heracles")
            writeBoulderState(directory, newState)
...
Ask the user which plan to work on. Present the options above and wait for their response.
        ...
        return `${EXECUTE_PLAN_TEMPLATE}\n\n---\n${contextInfo}`.replace(/\$SESSION_ID/g, sessionId).replace(/\$TIMESTAMP/g, timestamp)

      } catch (e) {
        return `Error resolving execute-plan context: ${e instanceof Error ? e.message : String(e)}`
      }
    },
  })
