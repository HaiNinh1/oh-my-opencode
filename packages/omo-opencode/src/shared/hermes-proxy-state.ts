 ...
 * - chat-message handler: first-message AgentPart parsing to set proxy target
 * - hermes-routing-guard: enforce pinned session routing and rewrite task() args
 * - tool-execute-after handler: capture child session ID on first successful task()
 * - event handler: cleanup on session.deleted
 *
 * Survives across turns and compaction in-process. Does NOT survive plugin restart.
 *
 * NOTE: The population side (setTarget / pinChildSession / resetTurnFlag / clear)
 * is wired from the shared plugin dispatch files (chat-message, tool-execute-after,
 * event). The two Hermes hooks in src/hooks/hermes-* only READ this state. When the
 * dispatch wiring is absent, every get() returns undefined and the proxy-specific
 * branches become no-ops, leaving the routing guard's non-proxy enforcement intact.
 */
